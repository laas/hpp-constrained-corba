...
class Client:
  """
  Connect and create clients for hpp-constrained-planner library.
  """
  def __init__(self):
    """
    ...
    name = [CosNaming.NameComponent ("hpp", "plannerContext"),
            CosNaming.NameComponent ("hpp", "constrained")]
    
    try:
        obj = self.rootContext.resolve (name)
    except CosNaming.NamingContext.NotFound, ex:
        raise CorbaError ('failed to find constrained service.')
    try:
        client = obj._narrow (Constrained)
    except KeyError:
